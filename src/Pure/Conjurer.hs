...
import Pure.Elm.Component (View,HasFeatures)
...
resourcePages 
  :: forall _role a.  
    ...
    ) => WebSocket -> ResourceRoute _role a -> View
resourcePages ws = \case
...
resourceRoutes 
  :: forall _role a route. 
    ( Typeable a
    , Readable a, Creatable _role a, Updatable _role a, Listable a 
    ) => (ResourceRoute _role a -> route) -> Routing route ()
